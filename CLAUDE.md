...
3. **Pilot: Horizon** — leans heavily on theme blocks, so it stress-tests the wrapper pattern against block-composed pages.
...
- **`analytics: boolean | object`** (default `true`) — analytics bridge opt-out. Almost never set to `false` — disabling silently breaks merchant reporting. The shipped form widened past the boolean described in "Analytics bridge" below: six independent bridges (`shopify`, `customEvents`, `trekkie`, `standardEvents`, `ga4`, `dataLayer`), where `true` means `{ shopify: true, customEvents: true, trekkie: false, standardEvents: 'auto', ga4: false, dataLayer: false }`. `trekkie` is off by default on purpose — it calls the undocumented `ShopifyAnalytics.lib.page`. **See the README's "Analytics & tracking" section for the current contract.**
...
3. **`customEvents`** (on by default) — the one publish path that DOES reach pixels. Standard *names* are fenced; prefixed **custom** events are documented as publishable from theme Liquid and delivered to "all custom pixels and app pixels". Pusha emits `pusha:page_viewed` plus prefixed copies of the serialized page-type payloads. Inert until a companion pixel forwards them — `docs/analytics-companion-pixel.md`. Does NOT revive third-party app pixels (they have no mapping for a prefixed name).
4. **`trekkie`** (OFF by default) — admin reporting only, a different pipe from pixels. `ShopifyAnalytics.lib.page(null, { path, url, pageType, resourceId })` on a swap lands `pageType`/`resourceId` in `trekkie_storefront_page_view` and both `storefront_customer_tracking` schemas, and WPM mirrors it into `storefront_customer_tracking_parity` — measured, `experiments/monorail-admin-probe.md`. Identity comes from a theme-rendered `<script data-pusha-trekkie-page>` block; **`ShopifyAnalytics.meta` is read-never-write** (other scripts read it). Off by default because the global is undocumented and outside Shopify's Liquid compatibility guarantee; optional-chained so its removal undercounts rather than corrupts. ⚠ Proves the event is *sent*, not that admin *counts* it.

**One emission, two destinations.** A hard-load pageview carries the same `event_id` across `trekkie_storefront_page_view`, `storefront_customer_tracking`, and `web_pixels_manager_event_publish` — admin and pixels are not independent systems, they're one emission fanned out. WPM observes a `lib.page` soft-nav call and forwards it to Monorail while declining to publish it to the pixel sandboxes.
...
**What's unmeasured:** whether installed apps actually listen. Sanctioned is not the same as consumed. That is the primary hypothesis of the probe rig (`pusha-probe`, variant J) and it decides how the Bucket X runtime half ships.
